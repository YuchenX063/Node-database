...
export class ComparisonDashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subs.push(this._api.getTypeRequest('stats/composition').subscribe({
      next: (res: any) => {
  ...
  private queryFor(scope: Scope): string {
    if (scope.kind === 'diocese') return 'stats/composition?diocese=' + encodeURIComponent(scope.place);
    if (scope.kind === 'state') return 'stats/composition?state=' + encodeURIComponent(scope.place);
    return 'stats/composition';
  }
  ...
  private render(): void {
    ...
    g.append('text').attr('transform', 'rotate(-90)')
      ...
      .text(isShare ? 'Share of the scope’s institutions' : 'Institutions recorded');
          ...
          .on('mousemove', (event: MouseEvent) => {
            ...
            tooltip.style('opacity', '1')
              ...
              .html(`<strong>${this.scopeLabel(scope)}</strong><br>` +
                `${this.labels[bucket]}: ${v.count} (${(v.share * 100).toFixed(1)}%)`);
          })
