...
export class CompositionDashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Readable labels and a fixed color per function bucket
  private readonly labels: Record<string, string> = {
    'religious institutions': 'Religious (parishes, missions)',
  ...
  private render(): void {
    ...
    // Time-positioned x-axis so the 1841-1859 gap shows as real empty space,
    ...
    // Shade the coverage gap between the two year-islands
    ...
    g.append('text')
      ...
      .text(isShare ? 'Share of recorded institutions' : 'Institutions recorded');
          ...
          .on('mousemove', (event: MouseEvent) => {
            ...
            tooltip.style('opacity', '1')
              ...
              .html(`<strong>${year} &middot; ${this.labels[bucket] ?? bucket}</strong><br>` +
                `${row!.count} institution${row!.count === 1 ? '' : 's'} &middot; ${(row!.share * 100).toFixed(1)}% of ${total}`);
          })
