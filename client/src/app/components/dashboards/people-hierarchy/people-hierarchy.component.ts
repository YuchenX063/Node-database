...
@Component({
  ...
  templateUrl: './people-hierarchy.component.html',
  styleUrl: './people-hierarchy.component.scss'
})
export class PeopleHierarchyDashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  @ViewChild('chart', { static: false }) chartRef!: ElementRef<HTMLDivElement>;

  loading = true;
  error: string | null = null;
  note = '';
  // Counts read better than shares here — the leadership ranks are tiny next to
  // the priests, so the pyramid's growth is clearest as raw numbers.
  mode: 'share' | 'count' = 'count';
  ...
  // Clergy rank labels and a high-to-low color ramp
  private readonly labels: Record<string, string> = {
    'archbishop': 'Archbishops (Most Rev.)',
    'bishop': 'Bishops (Rt. Rev.)',
    'vicar or superior': 'Vicars & superiors (Very Rev.)',
    'priest': 'Priests (Rev.)',
    'seminarian or lay': 'Seminarians & lay (Mr.)',
    'other': 'Other / unrecorded'
  };
  private readonly colors: Record<string, string> = {
    'archbishop': '#5b2a86',
    'bishop': '#9b4f9e',
    'vicar or superior': '#3c6e9e',
    'priest': '#3f9e7a',
    'seminarian or lay': '#e0a83b',
    'other': '#9aa0a6'
  };
  ...
  fetchData(): void {
    this.loading = true;
    this.error = null;
    const params: string[] = ['entity=people', 'dimension=title'];
    ...
    this.fetchSubscription = this._api.getTypeRequest(url).subscribe({
      ...
      error: (err: any) => {
        this.error = err?.error?.message || 'Could not load clergy data.';
        this.loading = false;
      }
    });
  }
  ...
  private render(): void {
    ...
    g.append('text')
      ...
      .text(isShare ? 'Share of clergy recorded' : 'Clergy recorded');
